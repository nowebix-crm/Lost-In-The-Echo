...
const verifyToken = (token: string, secret: string): Promise<{ status: TokenStatus, message: string }> => {
    return new Promise((resolve, reject) => {
        jwt.verify(token, secret, err => {
            if (err) {
                return reject({ status: TokenStatus.INVALID, message: err.message });
            }
            return resolve({ status: TokenStatus.VALID, message: 'Token is valid' });
        });
    });
};
